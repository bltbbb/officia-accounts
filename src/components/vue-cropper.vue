  ...
  <div class="vue-cropper" ref="cropper" @mouseover="scaleImg" @mouseout="cancleScale">
    ...
    <div
      ...
      :style="{
				...
				<span class="cropper-view-box" style="position: relative;" ref="imgBox">
					<img
...
//        })
      ...
      // 改变大小函数
      changeSize (e) {
        if(this.$refs.croBox.offsetTop > 0){
          this.moveTop = 0
        }
        if(this.moveLeft > 0){
          this.moveLeft= 0
      ...
      showPreview() {
        ...
        obj.img = {
          'width': this.trueWidth*this.scale + 'px',
          'height': this.trueHeight*this.scale + 'px',
          'left': this.moveLeft + 'px',
          'top': this.moveTop + 'px',
          'transform': 'rotateZ('+ this.rotate * 90 + 'deg)'
        }
        obj.w = this.cropW
        obj.h = this.cropH
        obj.url = this.imgs
